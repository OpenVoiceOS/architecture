...
OVOS-INTENT-3 §5, or a vocabulary name per OVOS-INTENT-3 §4), and
the §3.1-selected entry for `k` has a non-null `value`, the engine
**MUST** offer that value to its matcher as a **candidate** for
slot `k` **before** matching, and report the resolved value in
`Match.slots[k]` (keyed by `k`, unprefixed, regardless of whether
§3.1 selected a private or shared entry).
...
name in its definition. A `requires_context` key whose §3.1-selected
entry has a `null` value (a flag), or with no matching slot or
vocabulary name, is **gated only** (§6) — no candidate is injected.
...
- apply the §7 context-supplied slot rule when a
  `requires_context` key also names a slot or vocabulary of the
  intent's definition — offering the non-null entry value to the
  matcher as a candidate for that slot **before** matching, and
  letting an utterance-produced value for the same slot replace it;
