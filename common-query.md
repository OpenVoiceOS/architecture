...
Each contest carries a `query_id` generated by the plugin.
`session_id` already separates peers and conversations, and the
routing pair already carries the peer — neither separates **two
contests inside one session**, which is exactly what a repeated
question or a shared default session produces. OVOS-MSG-1 §5.4
provides no central correlation and directs a component needing
finer discrimination to carry it itself. `query_id` is that carrier,
and it is the only field this specification adds for it.
