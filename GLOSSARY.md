...
| **Contest identifier** | The correlation that ties every Message of one common-query round together — `context.utterance_id`, stamped once at the lifecycle source and carried onto every derived Message ([COMMON-QUERY-1 §6.4](common-query.md)). |
...
| **Willingness contest** | The candidate poll fallback runs over the effective handler pool on the `ovos.fallback.ping` / `ovos.fallback.pong` topic pair to decide which fallback skill claims the utterance ([FALLBACK-1 §6.1](fallback.md)). |
...
| **Candidate poll** | The shared ping/pong exchange a matching strategy uses to ask a set of candidates whether any of them claims the utterance, then decide from the answers ([PIPELINE-1 §4.5](pipeline-1.md)). |
