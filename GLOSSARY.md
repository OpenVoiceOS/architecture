...
| **Pipeline plugin** | A black-box component identified by an opaque `pipeline_id` that consumes utterances (and optionally registration messages) and returns intent matches. Loaded by the orchestrator at startup; ordered list lives in `session.pipeline_stages` (OVOS-PIPELINE-1). |
