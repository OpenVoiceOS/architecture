...
- **Slot-only template**, in an **input-direction** template (§2) only — a
  template that is a single named slot and nothing else (`{name}`). An
  input-direction template MUST carry at least one literal word; a bare slot
  ...
  The anchoring problem belongs to training and to match-time fill. §6 puts
  a `.dialog` outside both: it is output-direction and caller-filled (§5.1),
  and no intent engine trains on it. A bare `{day}` is rendered, not
  matched: the caller supplies the value for `day` and the renderer puts it
  in place of the name, which is the terse spoken answer and is the natural
  short reply in many languages. The line also satisfies §5.5, because
  `{day}` declares the same slot set as `It is {day}` and may sit beside it
  in one file. A tool MUST NOT reject an output-direction template because
  it carries no literal word.
...
defined over the expanded sample set of an input-direction template, a
naive implementation must expand the full template first — potentially expensive (§4.3). A tool MAY instead
...
| `language` | An object `{"code": <string>, "name": <string or null>}`, where `code` is a BCP-47 language tag in lowercase for the language the surface names, and `name` is the autonym of that language (the name of the language in that language itself) when one is known and `null` otherwise. When the tag has a script or a region subtag, `name` is the autonym of the primary language subtag only, and the script and the region stay in `code`. `name` does not depend on the utterance's language: the user's own words stay in the entry's `surface`. Example: for "speak in German" the entry has `"surface": "German"` and `"value": {"code": "de", "name": "Deutsch"}`; for "fala em japonês" it has `"surface": "japonês"` and `"value": {"code": "ja", "name": "日本語"}`; for "speak Brazilian Portuguese" it has `"surface": "Brazilian Portuguese"` and `"value": {"code": "pt-br", "name": "Português"}`. |
...
| `timezone` | An object `{"tz": <string>}`, where `tz` is an IANA time zone name. The surface is a zone name or a common zone abbreviation in the utterance's language. One surface gives one entry with one zone, also when the surface names more than one zone. When the session's zone (OVOS-SESSION-1 §3.5 `location.tz`, and the deployment's configured zone when the session declares none) uses that abbreviation or zone name, `tz` is the session's zone. Otherwise `tz` is the zone that the engine's zone table for the utterance's language gives for the surface. (One entry per zone is not permitted: in tzdata 38 zones use `CET` and 35 use `EST`, and a consumer has no rule to select one of them. The session's zone is the zone that `date` already resolves in.) Example: in a session with `location.tz` `Europe/Lisbon`, which does not use CET, for "what time is it in Central European Time" the entry has `"surface": "Central European Time"` and the table gives `"value": {"tz": "Europe/Paris"}`; in a session with `location.tz` `America/Detroit`, which uses EST, for "3 pm EST" it has `"surface": "EST"` and `"value": {"tz": "America/Detroit"}`. |
